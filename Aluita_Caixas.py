    ...
    for cashier in cashiers:
        ...
        for file in os.listdir(mainPath):            
            if file.lower().endswith('.xlsx') and all(word.lower() in file.lower() for word in cashier_filter) and config['config']['edited_name'].lower() not in file.lower():
                ...
                df = pd.read_excel(mainPath + file, sheet_name=None, header=None, usecols='A:F')
                ...
                for sheet in df:
                    ...
                    if df[sheet].empty == False:
                        ...
                        # If date is not None
                        if date is not None:
                            receiptsTotalsInserted = 0
                            receiptsTotals = 0
                            paymentsTotalsInserted = 0
                            paymentsTotals = 0
                            ...
                            for row in df[sheet]._values:
                                ...
                                if len(row) > 0:
                                    ...
                                    if cashier == 'POA':
                                        poaType = getParamInSectionWithFilters('POA', firstColumn)
                                        if poaType == 'payments':
                                        ...
                                        paymentTitle = getParamInSectionWithFilters('payments', firstColumn)
                                        receiptTitle = getParamInSectionWithFilters('receipts', firstColumn)
                                        totalTitle = getParamInSectionWithFilters('totals', firstColumn)
                                        title = getParamInSectionWithFilters('titles', firstColumn)
                                    ...
                                    #If column is not a title and valueType is not empty and valueTitle is not empty
                                        ...
                                        description = " ".join([valueTitle, firstColumn, (str(row[1]) if len(row) == 3 else '')])
                                        #value = last column
                                        value = row[-1]
                                        ...
                                        if description != "" and value != "" and type(value) is float and str(firstColumn) != str(value):
                                            #push to compact 'date' 'valueType' 'valueTitle' 'description' 'value'
                                            compact.append({
                                                'date': date,
                                                'valueType': valueType,
                ...
                compact.to_csv(mainPath + file.replace('.xlsx', '.csv'), sep=';', index=False)
